...
def extract_patient(drop_create=False):
    ...
    insert_query = text(
        "INSERT INTO _patient (patient_id, tribe, creator, date_created, changed_by, date_changed, voided, voided_by, date_voided, void_reason) VALUES (:patient_id, :tribe, :creator, :date_created, :changed_by, :date_changed, :voided, :voided_by, :date_voided, :void_reason)")
    ...
    with source_engine.connect() as source_conn:
        ...
        with target_engine.connect() as target_conn:
            for row in result:
                batch.append({
                    "patient_id": row.patient_id, "tribe": row.tribe, "creator": row.creator,
                    "date_created": row.date_created, "changed_by": row.changed_by, "date_changed": row.date_changed,
                    "voided": row.voided, "voided_by": row.voided_by, "date_voided": row.date_voided,
...
def extract_patient_identifier(drop_create=False):
    ...
    with source_engine.connect() as source_conn:
        ...
        with target_engine.connect() as target_conn:
            for row in result:
                batch.append({
                    "patient_identifier_id": row.patient_identifier_id, "patient_id": row.patient_id,
                    "identifier": row.identifier, "identifier_type": row.identifier_type, "preferred": row.preferred,
                    "location_id": row.location_id, "creator": row.creator, "date_created": row.date_created,
                    "voided": row.voided, "voided_by": row.voided_by, "date_voided": row.date_voided,
                    "void_reason": row.void_reason, "uuid": row.uuid
                })
                count += 1
                if len(batch) >= BATCH_SIZE:
                    info(
                        f"Inserting batch {batch_number} of {len(batch)} records into target _patient_identifier table...")
                    target_conn.execute(insert_query, batch)
                    target_conn.commit()
                    batch = []
                    batch_number += 1
...
def extract_patient_identifier_type(drop_create=False):
    ...
    insert_query = text(
        "INSERT INTO _patient_identifier_type (patient_identifier_type_id, name, description, format, check_digit, creator, date_created, required, format_description, validator, retired, retired_by, date_retired, retire_reason, uuid) VALUES (:patient_identifier_type_id, :name, :description, :format, :check_digit, :creator, :date_created, :required, :format_description, :validator, :retired, :retired_by, :date_retired, :retire_reason, :uuid)")
    with source_engine.connect() as source_conn:
        ...
        with target_engine.connect() as target_conn:
            for row in source_data:
                target_conn.execute(insert_query, {
                    "patient_identifier_type_id": row.patient_identifier_type_id, "name": row.name,
                    "description": row.description, "format": row.format, "check_digit": row.check_digit,
                    "creator": row.creator, "date_created": row.date_created, "required": row.required,
                    "format_description": row.format_description, "validator": row.validator, "retired": row.retired,
...
def extract_patient_program(drop_create=False):
    ...
    with source_engine.connect() as source_conn:
        ...
        with target_engine.connect() as target_conn:
            for row in result:
                batch.append({
                    "patient_program_id": row.patient_program_id, "patient_id": row.patient_id,
                    "program_id": row.program_id, "date_enrolled": row.date_enrolled,
                    "date_completed": row.date_completed, "location_id": row.location_id, "creator": row.creator,
                    "date_created": row.date_created, "changed_by": row.changed_by, "date_changed": row.date_changed,
                    "voided": row.voided, "voided_by": row.voided_by, "date_voided": row.date_voided,
                    "void_reason": row.void_reason, "uuid": row.uuid
                })
                ...
                if len(batch) >= BATCH_SIZE:
                    info(
                        f"Inserting batch {batch_number} of {len(batch)} records into target _patient_program table...")
                    target_conn.execute(insert_query, batch)
                    target_conn.commit()
                    batch = []
                    batch_number += 1
...
def extract_person(drop_create=False):
    ...
    with source_engine.connect() as source_conn:
        ...
        with target_engine.connect() as target_conn:
            for row in result:
                batch.append({
                    "person_id": row.person_id, "gender": row.gender, "birthdate": row.birthdate,
                    "birthdate_estimated": row.birthdate_estimated, "dead": row.dead, "death_date": row.death_date,
                    "cause_of_death": row.cause_of_death, "creator": row.creator, "date_created": row.date_created,
                    "changed_by": row.changed_by, "date_changed": row.date_changed, "voided": row.voided,
...
def extract_person_address(drop_create=False):
    ...
    with source_engine.connect() as source_conn:
        ...
        with target_engine.connect() as target_conn:
            for row in result:
                batch.append({
                    "person_address_id": row.person_address_id, "person_id": row.person_id, "preferred": row.preferred,
                    "address1": row.address1, "address2": row.address2, "city_village": row.city_village,
                    "state_province": row.state_province, "postal_code": row.postal_code, "country": row.country,
                    "latitude": row.latitude, "longitude": row.longitude, "creator": row.creator,
                    "date_created": row.date_created, "voided": row.voided, "voided_by": row.voided_by,
                    "date_voided": row.date_voided, "void_reason": row.void_reason,
                    "county_district": row.county_district, "address3": row.neighborhood_cell, "address4": row.region,
                    "address5": row.subregion, "address6": row.township_division, "uuid": row.uuid
                })
                count += 1
                if len(batch) >= BATCH_SIZE:
...
def extract_person_attribute(drop_create=False):
    ...
    with source_engine.connect() as source_conn:
        ...
        with target_engine.connect() as target_conn:
            for row in result:
                batch.append({
                    "person_attribute_id": row.person_attribute_id, "person_id": row.person_id, "value": row.value,
                    "person_attribute_type_id": row.person_attribute_type_id, "creator": row.creator,
                    "date_created": row.date_created, "changed_by": row.changed_by, "date_changed": row.date_changed,
                    "voided": row.voided, "voided_by": row.voided_by, "date_voided": row.date_voided,
                    "void_reason": row.void_reason, "uuid": row.uuid
                })
                ...
                if len(batch) >= BATCH_SIZE:
                    info(
                        f"Inserting batch {batch_number} of {len(batch)} records into target _person_attribute table...")
                    target_conn.execute(insert_query, batch)
                    target_conn.commit()
                    batch = []
                    batch_number += 1
...
def extract_person_attribute_type(drop_create=False):
    ...
    insert_query = text(
        "INSERT INTO _person_attribute_type (person_attribute_type_id, name, description, format, foreign_key, searchable, creator, date_created, changed_by, date_changed, retired, retired_by, date_retired, retire_reason, edit_privilege, sort_weight, uuid) VALUES (:person_attribute_type_id, :name, :description, :format, :foreign_key, :searchable, :creator, :date_created, :changed_by, :date_changed, :retired, :retired_by, :date_retired, :retire_reason, :edit_privilege, :sort_weight, :uuid)")
    with source_engine.connect() as source_conn:
    ...
    if source_data:
        ...
        with target_engine.connect() as target_conn:
            for row in source_data:
                target_conn.execute(insert_query, {
                    "person_attribute_type_id": row.person_attribute_type_id, "name": row.name,
                    "description": row.description, "format": row.format, "foreign_key": row.foreign_key,
                    "searchable": row.searchable, "creator": row.creator, "date_created": row.date_created,
                    "changed_by": row.changed_by, "date_changed": row.date_changed, "retired": row.retired,
                    "retired_by": row.retired_by, "date_retired": row.date_retired, "retire_reason": row.retire_reason,
                    "edit_privilege": row.edit_privilege, "sort_weight": row.sort_weight, "uuid": row.uuid
                })
            target_conn.commit()
        info("Import completed successfully.")
    else:
...
def extract_person_name(drop_create=False):
    ...
    with source_engine.connect() as source_conn:
        ...
        with target_engine.connect() as target_conn:
            for row in result:
                batch.append({
                    "person_name_id": row.person_name_id, "preferred": row.preferred, "person_id": row.person_id,
                    "prefix": row.prefix, "given_name": row.given_name, "middle_name": row.middle_name,
                    "family_name_prefix": row.family_name_prefix, "family_name": row.family_name,
                    "family_name2": row.family_name2, "family_name_suffix": row.family_name_suffix,
                    "degree": row.degree, "creator": row.creator, "date_created": row.date_created,
                    "voided": row.voided, "voided_by": row.voided_by, "date_voided": row.date_voided,
                    "void_reason": row.void_reason, "changed_by": row.changed_by, "date_changed": row.date_changed,
                    "uuid": row.uuid
                })
                count += 1
                if len(batch) >= BATCH_SIZE:
...
def load_patient():
    ...
    with target_engine.connect() as conn:
        ...
        conn.execute(text("""
            INSERT IGNORE INTO patient (patient_id, creator, date_created, changed_by, date_changed, voided, voided_by, date_voided, void_reason)
...
def load_patient_identifier_type():
    ...
    with target_engine.connect() as conn:
        ...
        conn.execute(text("""
        INSERT IGNORE INTO patient_identifier_type (patient_identifier_type_id, name, description, format, check_digit, creator, date_created, required, format_description, validator, location_behavior, retired, uuid)
...
def load_patient_identifier():
    ...
    with target_engine.connect() as conn:
        ...
        set_foreign_key_checks(target_engine, False)
        conn.execute(text("""
        ...
        """))
        conn.commit()
        set_foreign_key_checks(target_engine, True)
...
def load_patient_program():
    ...
    with target_engine.connect() as conn:
        ...
        ON DUPLICATE KEY UPDATE
            ...
            location_id = VALUES(location_id),
            outcome_concept_id = VALUES(outcome_concept_id),
            creator = VALUES(creator),
            date_created = VALUES(date_created),
            changed_by = VALUES(changed_by),
            date_changed = VALUES(date_changed),
